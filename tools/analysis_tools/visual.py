...
def lidiar_render(sample_token, data, out_path=None):
    ...
    visualize_sample(nusc, sample_token, gt_annotations,
                     pred_annotations, savepath=out_path+'_bev')
...
def render_sample_data(
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            if with_anns:
                for box in boxes_pred:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[j, ind], view=camera_intrinsic,
                               normalize=True, colors=(c, c, c))
                for box in boxes_gt:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[j + 2, ind], view=camera_intrinsic,
                               normalize=True, colors=(c, c, c))
    ...
    if out_path is not None:
        plt.savefig(out_path+'_camera', bbox_inches='tight',
...
if __name__ == '__main__':
    nusc = NuScenes(version='v1.0-mini',
                    dataroot='./data/V2X-Sim-2.0', verbose=True)
    # render_annotation('7603b030b42a4b1caa8c443ccc1a7d52')
    bevformer_results = mmcv.load(
        './test/tiny_4_cam_weight/Thu_Sep__7_19_17_35_2023/pts_bbox/results_nusc.json')
